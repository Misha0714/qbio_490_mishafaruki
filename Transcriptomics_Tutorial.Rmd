...
Use the dim() function to find the dimensions of the colData, rowRanges@elementMetadata, and unstranded dataframes. Record them below:
  ...
  ~ colData (rows, cols): 473 (patients), 175 (features) 
...
#!is.na(rna_se@colData$age_at_index)
```
...
primary_site_mask <- ifelse(colnames(rna_clinical) == "primary site", F, T)
rna_clinical <- rna_clinical[, primary_site_mask]

disease_type_mask <- ifelse(colnames(rna_clinical) == "disease type", F, T)
