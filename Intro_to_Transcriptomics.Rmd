...
```{r}
 knitr::opts_knit$set(root.dir = normalizePath("/Users/mishafaruki/Desktop/QBIO490/qbio_490_mishafaruki/inanalysis_data"))
```
...
#removal of "nested lists":
unnest(rna_clinical, cols = NestedColumn)
...
rownames(rna_genes) <- new_gene_names
rownames(rna_genes)
```
...
rownames(rna_counts) <- new_gene_names
colnames(rna_counts) <- new_sample_names

# Display the updated row and column names
rownames(rna_counts) <- new_gene_names
colnames(rna_counts) <- new_sample_names
rownames(rna_counts)
colnames(rna_counts)
...
# Assuming you have a SummarizedExperiment object with clinical data and gene counts
# Replace 'rna_se' with your actual SummarizedExperiment object
...
design_matrix <- model.matrix(~ Gender + Age_Category + Vital_Status, data = rna_clinical)

# Filter out genes with total expression across all patients less than 10
filtered_genes <- rowSums(assay(rna_se)) >= 10
rna_se_filtered <- rna_se[filtered_genes, ]

# Ensure that the clinical data is in the same order as the expression data
rna_clinical <- rna_clinical[row.names(rna_se_filtered), ]

# Create a linear model
fit <- lmFit(rna_se_filtered, design_matrix)

# Contrast: Compare males vs females, controlling for age_category or vital_status
contrast_matrix <- makeContrasts(GenderMale - GenderFemale, levels = design_matrix)
...
# Get the results
results <- topTable(fit_contrasts, coef = 1)  # Adjust the coefficient index if needed
